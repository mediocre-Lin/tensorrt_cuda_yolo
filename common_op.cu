...
__global__ void Batch_ROI_BGR2RGB_NORM_Kernel_2(const uchar3* in, float* out,
                                                int batch, int* roi_x,
                                                int* roi_y, int* roi_width,
                                                int* roi_height, float* means,
                                                float* stds) {
  const int x = blockIdx.x * blockDim.x + threadIdx.x;
  const int y = blockIdx.y * blockDim.y + threadIdx.y;
  if ((x >= roi_width[0]) || (y >= roi_height[0])) return;
...
__global__ void Batch_ROI_BGR2RGB_NORM_Kernel(
    cv::cuda::PtrStepSz<uchar3> in, float* out, int batch,  int* roi_x,
...
__global__ void ROI_BGR2RGB_NORM_Kernel(cv::cuda::PtrStepSz<uchar3> in,
                                        float* out, int roi_x, int roi_y,
...
__global__ void toNCHW_Norm_Kernel(cv::cuda::PtrStepSz<uchar3> in, float* out,
                                   float* means, float* stds) {
  const int x = blockIdx.x * blockDim.x + threadIdx.x;
  const int y = blockIdx.y * blockDim.y + threadIdx.y;

  if ((x >= in.cols) || (y >= in.rows)) return;
  uchar3 v = in(y, x);
...
extern "C" void toNCHW_Norm(const cv::cuda::GpuMat& input, float* output,
  ...
  const dim3 grid((input.cols + threads.x - 1) / threads.x,
  ...
  cudaMemcpyAsync(d_means, means, 3 * sizeof(float), cudaMemcpyHostToDevice,
                  stream);
  cudaMemcpyAsync(d_stds, stds, 3 * sizeof(float), cudaMemcpyHostToDevice,
                  stream);
  toNCHW_Norm_Kernel<<<grid, threads, 0, stream>>>(input, output, d_means,
                                                   d_stds);
  cudaFree(d_means);
  cudaFree(d_stds);
}
...
extern "C" void ROI_BGR2RGB_NORM(const cv::cuda::GpuMat& input, float* output,
  ...
  const dim3 threads(32, 8);
  const dim3 grid((roi_width + threads.x - 1) / threads.x,
                  (roi_height + threads.y - 1) / threads.y);
  float* d_means = nullptr;
  float* d_stds = nullptr;
  cudaMalloc(&d_means, 3 * sizeof(float));
  cudaMalloc(&d_stds, 3 * sizeof(float));
  ...
  cudaMemcpyAsync(d_stds, stds, 3 * sizeof(float), cudaMemcpyHostToDevice,
                  stream);
  ROI_BGR2RGB_NORM_Kernel<<<grid, threads, 0, stream>>>(
      input, output, roi_x, roi_y, roi_width, roi_height, d_means, d_stds);
  cudaFree(d_means);
  cudaFree(d_stds);
}
extern "C" void Batch_ROI_BGR2RGB_NORM(const cv::cuda::GpuMat& input,
  ...
  //                         cudaMemcpyHostToDevice, stream);
  Batch_ROI_BGR2RGB_NORM_Kernel<<<grid, threads, 0, stream>>>(
      input, output, batch, d_x, d_y, d_w, d_h, d_means, d_stds);
  // cudaFree(d_stds);
  // cudaFree(d_means);
  // cudaFree(d_x);
  // cudaFree(d_y);
...
extern "C" void Batch_ROI_BGR2RGB_NORM_2(const uchar3* input, float* output,
                                         ...
                                         cudaStream_t stream) {
  const dim3 threads(32, 8);
  const dim3 grid((rois[0].width + threads.x - 1) / threads.x,
                  (rois[0].height + threads.y - 1) / threads.y);
  int* h_x = new int[rois.size()];
  int* h_y = new int[rois.size()];
  int* h_w = new int[rois.size()];
  int* h_h = new int[rois.size()];
  ...
  cudaMemcpyAsync(d_stds, stds, 3 * sizeof(float), cudaMemcpyHostToDevice,
                  stream);
  Batch_ROI_BGR2RGB_NORM_Kernel_2<<<grid, threads, 0, stream>>>(
      input, output, batch, d_x, d_y, d_w, d_h, d_means, d_stds);
  cudaFree(d_stds);
  cudaFree(d_means);
  cudaFree(d_x);
